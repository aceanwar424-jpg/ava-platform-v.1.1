  ...
  if (url.pathname === '/apps/index.html') {
    ...
    content = content.replace('</body>', `<script>
      ...
      async function sbRpc(name){
        if(name==='wellness_personal_dashboard') return {programs:[{id:'synthetic-program',name:'Program Kardiometabolik',description:'Program uji sintetis',enrollment_status:'active',consent_status:'granted'}],observations:[{id:'o1',program_id:'synthetic-program',group_id:'g1',code:'blood_pressure_systolic',value:120,unit:'mmHg',measurement_context:'resting',measured_at:'2026-09-22T08:00:00+07:00',source:'self_reported',verification_status:'unverified'},{id:'o2',program_id:'synthetic-program',group_id:'g1',code:'blood_pressure_diastolic',value:80,unit:'mmHg',measurement_context:'resting',measured_at:'2026-09-22T08:00:00+07:00',source:'self_reported',verification_status:'unverified'}],open_tasks:[]};
        ...
        if(name==='wellness_admin_dashboard') return {corporates:[{id:10,name:'Synthetic Corporate'}],programs:[{id:'synthetic-program',code:'SYN-CARDIO',name:'Program Kardiometabolik',corporate_name:'Synthetic Corporate',status:'pilot',enrolled:24,observations:96,open_tasks:3}],reminder_rules:[],imports:[]};
        return [];
      }
