...
async function handleChatResponse(t: Task) {
  ...
  const sysPrompt = `Anda adalah ${agentData.name}, dengan peran sebagai ${agentData.role_title} di AVA.
...
async function notifyDrafts(results: Record<string, unknown>[]) {
  ...
  const text = `🤖 AVA Agentic: ${drafts.length} draft baru menunggu approval:\n` +
    drafts.map((d) => `• ${d.note}`).join('\n') + `\nBuka menu Agentic AI → Approval Inbox.`;
  try {
    await fetch(url, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ text, source: 'ava-agentic', count: drafts.length }),
    });
  } catch { /* non-fatal */ }
}
