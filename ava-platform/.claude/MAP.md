...
9. **DUA portal korporat, jangan tertukar** — `apps/index.html` (login akun, roster+booking+approval penuh) vs `portal_korporat.html` (tautan bertoken, tanpa akun, lebih ringkas). `config/domain.json` mengarahkan `corporate.avahealth.sbs` ke yang PERTAMA. Salah tunjuk = menu terlihat dangkal padahal fiturnya ada.
