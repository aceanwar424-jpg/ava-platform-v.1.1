...
(function () {
  ...
  const WELLNESS_NOTICE_VERSION = 'wellness-privacy-v2';
  const safe = value => window.WellnessFlow.escape(value);
  ...
  async function renderPersonalWellness() {
    ...
    try {
      ...
      const activePrograms = programs.filter(program => program.processing_allowed);
      const pendingPrograms = programs.filter(program => !program.processing_allowed && ['invited','active'].includes(program.enrollment_status));
      if (!programs.length) {
      ...
      const consentCards = pendingPrograms.map(program => `<section class="wellness-card wellness-consent-card">
        ...
        <p>HR yang berwenang di perusahaan Anda dapat melihat identitas, hasil pemeriksaan individual, riwayat, level operasional, kehadiran dan laporan treatment untuk evaluasi program. Admin pengelola HIS mengatur dan melaksanakan treatment. IHC hanya mengirim hasil pada program yang ditugaskan. Informasi program tidak menggantikan diagnosis atau konsultasi medis.</p>
      ...
      target.innerHTML = `
        ...
        <section id="wellness-personal-treatments" class="wellness-card"></section><div class="wellness-safety">Hasil pada halaman ini membantu pemantauan program dan bukan diagnosis. Bila merasa tidak sehat atau hasil sangat berbeda dari biasanya, hubungi IHC atau layanan darurat sesuai kondisi Anda.</div>`;
      await window.WellnessFlow.personal($('wellness-personal-treatments'), programs);
    } catch (error) {
  ...
  async function renderCorporateWellness() {
    ...
    if (target) await window.WellnessFlow.corporate(target);
  }
  ...
  async function renderWellnessAdmin() {
    ...
    if (!target) return;
    if (!$('his-wellness-program')) {
      target.innerHTML = '<section class="wellness-card"><h2>Pengelolaan program di HIS</h2><p>Konfigurasi, pelaksanaan sesi dan laporan treatment dikelola di HIS.</p><a href="https://his.avahealth.sbs/?view=his-wellness">Buka Orkestrasi Wellness di HIS</a></section>';
      return;
    }
  ...
  async function wellnessSaveProgram(event) {
    ...
    try {
      await rpc('wellness_admin_save_program', { p_data: {
        ...
        reporting_plan: { daily: 'his', weekly: 'authorized_hr', monthly: 'authorized_hr' }
      }});
  ...
  async function wellnessOpenImport(programId) {
    if ($('his-wellness-program')) document.querySelectorAll('[data-pane]').forEach(p => p.hidden = p.dataset.pane !== 'imports');
    else showView('wellness-import-view');
    await renderWellnessImport(programId);
  }

  async function renderWellnessImport(selectedProgram = null) {
    ...
    try {
      const data = await rpc('wellness_import_dashboard');
      const programs = data.programs || [];
      target.innerHTML = `<section class="wellness-heading"><div><span>IHC DATA INTAKE</span><h2>Bulk Upload Hasil IHC</h2><p>Unggah CSV tervalidasi. Setiap baris dicocokkan dengan Employee ID karyawan, diberi provenance IHC, dan dicatat dalam batch yang idempoten.</p></div><button class="wellness-secondary" onclick="wellnessDownloadTemplate()">Unduh template CSV</button></section>
      ...
      if (selectedProgram) $('wellness-import-program').value = selectedProgram;
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        const mapped = rows.map(r => {
          ...
          return {
            employee_id: obj.employee_id || obj.nik || '',
