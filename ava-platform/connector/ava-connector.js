#!/usr/bin/env node
// ══════════════════════════════════════════════════════════════════════
// AVA Lab Connector — Jembatan Alat Lab (LIS) ↔ Supabase
// ──────────────────────────────────────────────────────────────────────
...
// master "analyzers" di AVA GLOBAL ECOSYSTEM (isi IP/port/mode/protocol di UI).
//
// Tanpa dependensi eksternal. Node.js 18+ (butuh global fetch).
//   Jalankan:  node ava-connector.js
...
function hl7Ack(inMsg) {
  let ctrlId = '1', sendApp = 'AVA GLOBAL ECOSYSTEM', recvApp = 'ANALYZER';
...
async function sendAstmOrders(socket, orders) {
  ...
  if (!await kirim(`H|\\^&|||AVA GLOBAL ECOSYSTEM`, 'header')) { socket.write(Buffer.from([EOT])); return false; }
...
function sendHl7Orders(socket, analyzer, orders) {
  const ts = new Date().toISOString().replace(/[-:T]/g, '').slice(0, 14);
  orders.forEach((o) => {
    const seg = [`MSH|^~\\&|AVA GLOBAL ECOSYSTEM||${analyzer.code || ''}||${ts}||ORM^O01|${o.barcode}|P|2.3`,
...
async function loadAndBind() {
  ...
  if (!bound.size) log(`📋 ${cfgList.length} alat aktif dari AVA GLOBAL ECOSYSTEM`);
  cfgList.forEach(bindAnalyzer);
  if (!cfgList.length && !bound.size) log('ℹ Belum ada alat dengan IP+port+integrasi aktif. Set di AVA GLOBAL ECOSYSTEM → master Alat.');
}

// ── Status HTML lokal monitor ─────────────────────────────────────────
const STATUS_HTML = `<!doctype html><html lang="id"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1"><title>AVA Lab Connector — Status</title>
<style>
  :root {
    --bg-dark: #0A1120;
    --card-bg: #131E35;
