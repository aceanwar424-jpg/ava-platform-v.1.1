    ...
    ${t.status==='PUBLISHED' ? `<div style="margin-top:10px;background:#EFF6FF;border:1px solid #93C5FD;border-radius:8px;padding:9px 12px;font-size:12px;color:var(--ink-11)">
        ...
        ${t.agent==='DOCUMENT'
          ? `• Tab <a href="javascript:switchAgenticTab('docs')" style="font-weight:700">Dokumen QMS</a> (registry + nomor resmi & jadwal review)<br>
             • Menu <a href="javascript:navigate('wiki',{tab:'docs'})" style="font-weight:700">Wiki AVA → Dokumen Resmi</a> (bisa unduh .docx)`
...
async function agGenImageForTask(id){
  ...
  try{
    ...
    const prompt = copy.image_prompt ||
      `clean modern healthcare flyer visual about "${topic}", teal and navy AVA branding, laboratory equipment or abstract medical shapes, soft lighting, no people, no text`;
