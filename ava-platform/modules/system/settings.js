...
async function exportAllData() {
  ...
  try {
    ...
    a.download=`AVA_Backup_${new Date().toLocaleDateString('id-ID').replace(/\//g,'-')}.csv`;
  ...
  return `
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:14px">
      ...
      <div class="card">
        ...
        <div style="font-size:12px;color:var(--gray);margin-bottom:10px">
          Nama, alamat, logo, kontak AVA — dipakai di invoice, surat, voucher.
        </div>
...
async function openMasterServices() {
  openModal(`
    ...
    <div style="font-size:13px;color:var(--gray);margin-bottom:14px">
      Daftar layanan standar AVA — digunakan di Home Care, MCU, dan Voucher.
    </div>
...
async function openOrgSettings() {
  const orgName  = localStorage.getItem('ol_org_name')  || 'AVA Health & Lab Diagnostics by Plebo';
  ...
  const orgEmail = localStorage.getItem('ol_org_email') || 'info@ava.id';
