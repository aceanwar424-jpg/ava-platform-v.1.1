    ...
    <div style="display:grid;grid-template-columns:340px 1fr;gap:14px;align-items:start">
      <div class="wiki-card">
        ...
        <div class="form-group"><label>Call to Action</label>
          <input id="ct-cta" placeholder="mis. Hubungi WA AVA untuk jadwal MCU"></div>
...
async function runContentGen(){
  ...
  try{
    const system=`Kamu copywriter kesehatan untuk AVA (laboratorium klinik & layanan MCU/home care) di Indonesia.
  ...
  out.innerHTML=`<div class="wiki-card">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:10px">
      ${_mediaImages.map((src,i)=>`<div>
        ...
        <a class="btn btn-ghost btn-sm" style="margin-top:6px;display:block;text-align:center" href="${src}" download="ava_image_${i+1}.png">${svgIcon('download',13)} Download</a>
      </div>`).join('')}
    </div></div>`;
}
