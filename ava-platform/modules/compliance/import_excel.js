...
function downloadTemplate(key) {
  ...
  a.download = `template_${key}_ava.csv`;
  a.click();
...
function _downloadXLSX(key, tpl, headers, examples, notes) {
  ...
  XLSX.writeFile(wb, `template_${key}_ava.xlsx`);
...
function _downloadAllXLSX() {
  ...
  // Master info sheet
  const info = [
    ['AVA GLOBAL ECOSYSTEM — Master Template Import'],
  ...
  XLSX.writeFile(wb, 'ava_master_template.xlsx');
