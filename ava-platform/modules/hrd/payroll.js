...
// Seluruh tarif adalah PARAMETER, bukan angka yang dipatri di kode â€” karena
  ...
  document.getElementById('main-content').innerHTML = `
    <div class="page-header">
      ...
      <div class="btn-row">
        ...
        <button class="btn btn-ghost btn-sm" onclick="openEmployeeComponents()">ðŸ’¼ Tunjangan Karyawan</button>
      </div>
    </div>
    <div id="pay-warn"></div>
    <div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap;align-items:center">
      <input type="month" class="table-filter" id="pay-period" value="${payPeriod}" onchange="loadPayroll()">
      <button class="btn btn-teal btn-sm" onclick="runPayroll()">ðŸ§® Hitung Gaji Periode Ini</button>
...
async function loadPayrollSettings() {
  ...
  if (paySettings === null) {
    el.innerHTML = `<div class="status-box status-warn" style="margin-bottom:14px">
      Modul penggajian belum tersedia â€” jalankan <code>supabase_fase4b.sql</code>.</div>`;
    return;
  }
...
function paintPayroll() {
  ...
  if (st) st.innerHTML = final
    ? `<span style="color:var(--success-deep);font-weight:600">Final</span> Â· dikunci ${payRun.finalized_at ? new Date(payRun.finalized_at).toLocaleString('id-ID') : ''}`
    : `<span style="color:var(--warn-deep);font-weight:600">Draft</span> Â· masih bisa dihitung ulang`;
  ...
  el.innerHTML = `
    ...
    <div class="table-wrap"><table><thead><tr>
      ...
      <td style="font-weight:600">${i.employee_name || 'â€”'}</td>
      ...
      <td style="text-align:right;font-variant-numeric:tabular-nums;color:${i.commission ? 'var(--teal)' : 'var(--gray)'}">${i.commission ? formatCurrency(i.commission) : 'â€”'}</td>
...
async function runPayroll() {
  ...
  try {
    ...
    toast(`âœ… ${r?.employees || 0} karyawan dihitung â€” bruto ${formatCurrency(r?.gross || 0)}`, 'ok');
    await loadPayroll();
  } catch (e) {
    toast('âŒ ' + (/not find the function/i.test(e.message) ? 'Jalankan supabase_fase4b.sql dulu' : e.message), 'err');
  }
}
...
async function finalizePayroll(runId) {
  ...
  try {
    const res = await sbRpc('finalize_payroll', { p_run_id: runId });
    toast('Gaji difinalkan dan jurnal tercatat', 'ok');

    // â”€â”€ FINANCE BRIDGE: Posting beban gaji ke Akuntansi (Phase F) â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Sebelumnya: gaji dibayar tapi laporan L/R tidak berubah
    // Sekarang: beban gaji otomatis masuk buku besar
    try {
      const totalGross = res?.gross_total || res?.total_gross || 0;
      if (typeof postToLedger === 'function' && totalGross > 0) {
        const period = document.getElementById('pay-period')?.value || new Date().toISOString().slice(0,7);
        await postToLedger(
          'payroll.salary',
          totalGross,
          `Beban gaji karyawan periode ${period} (${res?.employees || 0} karyawan)`,
          'payroll_runs',
          runId
        );
      }
    } catch(eLedger) { console.warn('[Payroll] postToLedger skip:', eLedger.message); }
...
async function savePayrollSettings() {
  ...
  try {
    ...
    toast('âœ… Parameter tersimpan', 'ok');
...
async function openEmployeeComponents() {
  ...
  openModal(`
    <div class="modal-header"><div class="modal-title">ðŸ’¼ Tunjangan &amp; Potongan Karyawan</div>
      <button class="modal-close" onclick="closeModalForce()" style="font-size:10.5px;font-weight:700"></button></div>
    <div class="form-row">
    ...
    <div class="table-wrap" style="max-height:40vh;overflow:auto;margin-top:12px"><table><thead><tr>
      ...
      return `<tr>
        ...
        <td style="font-size:12.5px">${c.component || 'â€”'}</td>
...
async function addEmployeeComponent() {
  ...
  try {
    ...
    toast('âœ… Komponen ditambahkan', 'ok'); openEmployeeComponents();
  } catch (e) { toast('âŒ ' + e.message, 'err'); }
}
...
function printPayslip(itemId) {
  ...
  w.document.write(`<html><head><meta charset="utf-8"><title>Slip Gaji â€” ${i.employee_name}</title>
    ...
    <h2>Slip Gaji â€” ${org}</h2>
    <div class="sub">${i.employee_name} Â· Periode ${payPeriod}</div>
    <table>
...
// ═══════════════════════════════════════════════════════════════
// ENGINE PPh 21 TER (TARIF EFEKTIF RATA-RATA) SESUAI PMK 168/2023
// ═══════════════════════════════════════════════════════════════
function calculatePPh21TER(monthlyGross, terCategory = 'A') {
  const g = Math.max(0, monthlyGross || 0);

  // Matriks TER Kategori A (PTKP TK/0, TK/1, K/0)
  if (terCategory === 'A') {
    if (g <= 5400000) return { rate: 0.0, tax: 0, category: 'TER_A' };
    if (g <= 5650000) return { rate: 0.0025, tax: Math.round(g * 0.0025), category: 'TER_A' };
    if (g <= 5950000) return { rate: 0.005, tax: Math.round(g * 0.005), category: 'TER_A' };
    if (g <= 6300000) return { rate: 0.0075, tax: Math.round(g * 0.0075), category: 'TER_A' };
    if (g <= 6750000) return { rate: 0.01, tax: Math.round(g * 0.01), category: 'TER_A' };
    if (g <= 7500000) return { rate: 0.0125, tax: Math.round(g * 0.0125), category: 'TER_A' };
    if (g <= 8550000) return { rate: 0.015, tax: Math.round(g * 0.015), category: 'TER_A' };
    if (g <= 9650000) return { rate: 0.0175, tax: Math.round(g * 0.0175), category: 'TER_A' };
    if (g <= 10050000) return { rate: 0.02, tax: Math.round(g * 0.02), category: 'TER_A' };
    if (g <= 10350000) return { rate: 0.0225, tax: Math.round(g * 0.0225), category: 'TER_A' };
    if (g <= 10700000) return { rate: 0.025, tax: Math.round(g * 0.025), category: 'TER_A' };
    if (g <= 11050000) return { rate: 0.03, tax: Math.round(g * 0.03), category: 'TER_A' };
    if (g <= 11600000) return { rate: 0.035, tax: Math.round(g * 0.035), category: 'TER_A' };
    if (g <= 12500000) return { rate: 0.04, tax: Math.round(g * 0.04), category: 'TER_A' };
    if (g <= 13750000) return { rate: 0.05, tax: Math.round(g * 0.05), category: 'TER_A' };
    if (g <= 15100000) return { rate: 0.06, tax: Math.round(g * 0.06), category: 'TER_A' };
    if (g <= 16950000) return { rate: 0.07, tax: Math.round(g * 0.07), category: 'TER_A' };
    if (g <= 19750000) return { rate: 0.08, tax: Math.round(g * 0.08), category: 'TER_A' };
    if (g <= 24150000) return { rate: 0.09, tax: Math.round(g * 0.09), category: 'TER_A' };
    return { rate: 0.10, tax: Math.round(g * 0.10), category: 'TER_A' };
  }

  // Matriks TER Kategori B (PTKP TK/2, TK/3, K/1, K/2)
  if (terCategory === 'B') {
    if (g <= 6200000) return { rate: 0.0, tax: 0, category: 'TER_B' };
    if (g <= 6500000) return { rate: 0.0025, tax: Math.round(g * 0.0025), category: 'TER_B' };
    if (g <= 6850000) return { rate: 0.005, tax: Math.round(g * 0.005), category: 'TER_B' };
    if (g <= 7300000) return { rate: 0.0075, tax: Math.round(g * 0.0075), category: 'TER_B' };
    if (g <= 9200000) return { rate: 0.01, tax: Math.round(g * 0.01), category: 'TER_B' };
    if (g <= 10750000) return { rate: 0.015, tax: Math.round(g * 0.015), category: 'TER_B' };
    if (g <= 12550000) return { rate: 0.02, tax: Math.round(g * 0.02), category: 'TER_B' };
    if (g <= 14950000) return { rate: 0.04, tax: Math.round(g * 0.04), category: 'TER_B' };
    return { rate: 0.08, tax: Math.round(g * 0.08), category: 'TER_B' };
  }

  // Fallback Kategori C
  return { rate: 0.05, tax: Math.round(g * 0.05), category: 'TER_C' };
}

window.renderPayroll = renderPayroll;
window.runPayroll = runPayroll;
window.openPayrollSettings = openPayrollSettings;
window.openEmployeeComponents = openEmployeeComponents;
window.addEmployeeComponent = addEmployeeComponent;
window.deleteEmployeeComponent = deleteEmployeeComponent;
window.printPayslip = printPayslip;
window.calculatePPh21TER = calculatePPh21TER;
