...
//   2. Otomatis       → renderAnalyzerHub (dashboard: pesan dari AVA
...
async function renderAnalyzerHub(){
  ...
  if(msgs===null){
    host.innerHTML=`
      <div style="background:var(--warn-soft2);border:1px solid #FDE68A;border-radius:12px;padding:18px 20px;font-size:13px;color:var(--warn-deeper);line-height:1.7">
        ...
        <span style="font-size:12px;color:var(--warn-deep)">Setelah itu, jalankan <code>AVA Connector</code> di PC lab agar hasil alat masuk ke sini.</span>
      </div>`;
    return;
  }
  ...
  host.innerHTML=`
    ...
    <div class="card">
      ...
      }).join(''):`<tr><td colspan="6" style="text-align:center;padding:24px;color:var(--gray)">Belum ada pesan. Pastikan AVA Connector berjalan di PC lab dan alat mengirim hasil.</td></tr>`}
      </tbody></table></div>
    </div>`;
