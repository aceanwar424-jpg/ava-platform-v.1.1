// ═══════════════════════════════════════════════════════════════
// MODULE: Akuntansi (Fase 4) — Buku Besar, Neraca Saldo, Laba Rugi per Unit
  ...
  try {
    ...
    el.innerHTML = `<div class="status-box status-warn">
      ${/not find the function|does not exist/i.test(e.message)
        ? 'Modul akuntansi belum tersedia — jalankan <code>supabase_fase4.sql</code> di Supabase SQL Editor.'
        : e.message}</div>`;
  }
}
...
async function paintJournals(el) {
  ...
  el.innerHTML = `<div class="table-wrap"><table><thead><tr>
    ...
    <td style="font-family:ui-monospace,monospace;font-size:11.5px;color:var(--teal)">${e.entry_no || '—'}
      ${e.is_reversal ? '<span class="badge badge-gray" style="margin-left:4px">balik</span>' : ''}</td>
    <td style="font-size:11.5px;color:var(--gray)">${e.entry_date ? formatDateShort(e.entry_date) : '—'}</td>
    <td style="font-size:12.5px">${e.description || '—'}</td>
  ...
  openModal(`
    ...
    <table style="width:100%;font-size:12.5px"><thead><tr style="background:var(--bg2)">
      ...
      <td style="padding:6px"><span style="font-family:ui-monospace,monospace;color:var(--teal)">${l.account_code}</span>
        ${l.account_name}</td>
      <td style="padding:6px;text-align:right;font-variant-numeric:tabular-nums">${l.debit ? formatCurrency(l.debit) : '—'}</td>
      <td style="padding:6px;text-align:right;font-variant-numeric:tabular-nums">${l.credit ? formatCurrency(l.credit) : '—'}</td>
    </tr>`).join('')}</tbody></table>
...
function askReverseJournal(id) {
  openModal(`
    ...
    <div class="modal-footer">
      ...
      <button class="btn btn-danger" onclick="doReverseJournal(${id})">↩ Balik Jurnal</button>
    </div>`);
}
...
async function doReverseJournal(id) {
  ...
  try {
    ...
    toast('✅ Jurnal balik ' + (r?.entry_no || '') + ' tercatat', 'ok');
...
async function saveGLMappings() {
  ...
  try {
    ...
    toast('✅ Pemetaan tersimpan', 'ok'); closeModalForce();
  } catch (e) { toast('❌ ' + e.message, 'err'); }
}