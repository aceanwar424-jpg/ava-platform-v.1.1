...
class LLMAdapter {
  ...
  async generateCompletion(options) {
    ...
    try {
      ...
      } else if (this.provider === 'mock' && process.env.NODE_ENV !== 'production') {
        rawText = await this._callMock(prompt);
      } else {
        throw new Error(`Provider LLM tidak didukung atau mock tidak diizinkan: ${this.provider}`);
      }
      ...
      if (this.provider === 'mock' && process.env.NODE_ENV !== 'production') {
        ...
        return { text: rawText, sections: this.parseDelimiters(rawText), provider: 'mock', model: 'mock', timestamp: new Date().toISOString() };
      }
      throw new Error(`LLM provider ${this.provider} gagal: ${err.message}`, { cause: err });
    }
  }
