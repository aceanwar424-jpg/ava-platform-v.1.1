...
class ISO15189Checker {
  ...
  evaluateDocument(docContent, metadata = {}) {
    ...
    const negated = keyword => new RegExp(`(?:tidak|tanpa|belum|dilarang|no|not)\\s+(?:\\w+\\s+){0,3}${keyword.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'i').test(contentLower);
    const findings = [];
    let matchedCount = 0;

    ISO_15189_2022_CLAUSES.forEach(clause => {
      const matchedKeywords = clause.keywords.filter(kw => contentLower.includes(kw) && !negated(kw));
      const contradictedKeywords = clause.keywords.filter(kw => negated(kw));
      const isFulfilled = matchedKeywords.length > 0 && contradictedKeywords.length === 0;
      ...
      findings.push({
        ...
        status: isFulfilled ? 'FULFILLED' : (contradictedKeywords.length ? 'CONTRADICTED' : 'GAP_DETECTED'),
        matched_keywords: matchedKeywords,
        contradicted_keywords: contradictedKeywords,
        missing_keywords: isFulfilled ? [] : clause.keywords.filter(kw => !matchedKeywords.includes(kw)),
    ...
    const highGapCount = findings.filter(f => ['GAP_DETECTED', 'CONTRADICTED'].includes(f.status) && f.severity === 'HIGH').length;
