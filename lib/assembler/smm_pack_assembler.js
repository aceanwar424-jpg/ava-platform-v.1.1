...
class SMMPackAssembler {
  ...
  assemble(tenantConfig, selectedModuleIds = null) {
    ...
    const required = ['tenant_id', 'name', 'legal_entity', 'license_number'];
    const missing = required.filter(key => !(
      key === 'tenant_id' ? tenantConfig?.[key] : info[key]
    ));
    if (missing.length) {
      throw new Error(`Konfigurasi tenant tidak lengkap: ${missing.join(', ')}`);
    }
    ...
    const overallEval = this.checker.evaluateDocument(combinedContent, {
      title: `Paket SMM - ${tenantConfig.lab_info.name}`,
  ...
  _replaceVariables(text, tenantConfig) {
    ...
    replaced = replaced.replace(/\{\{LEGAL_ENTITY\}\}/g, info.legal_entity);
    replaced = replaced.replace(/\{\{LICENSE_NUMBER\}\}/g, info.license_number);
    replaced = replaced.replace(/\{\{EFFECTIVE_DATE\}\}/g, qms.effective_date || new Date().toISOString().slice(0, 10));
    replaced = replaced.replace(/\{\{CITY\}\}/g, info.city || '');
    replaced = replaced.replace(/\{\{LAB_DIRECTOR_NAME\}\}/g, qms.lab_director_name || '');
    replaced = replaced.replace(/\{\{LAB_DIRECTOR_SIP\}\}/g, qms.lab_director_sip || '');
